...
import './app.css';
import { setBridge, onBridgeReady } from '$lib/bridge-provider.ts';

// Detect Electron bridge (synchronous — preload sets window.tiltLauncher).
if ('tiltLauncher' in window) {
  setBridge(
    (window as unknown as Record<string, unknown>).tiltLauncher as import('@tilt-launcher/sdk').LauncherBridge,
  );
}
...
if ('__TAURI_INTERNALS__' in window) {
  Promise.all([
    ...
    setBridge({
      ...
      triggerResource: (envId: string, resourceName: string) =>
        invoke('trigger_resource', { envId, resourceName }),
      enableResource: (envId: string, resourceName: string) =>
        invoke('enable_resource', { envId, resourceName }),
      disableResource: (envId: string, resourceName: string) =>
        invoke('disable_resource', { envId, resourceName }),
      pickTiltfile: async () => {
      ...
      classifyTiltfilePath: (filePath: string) =>
        invoke('classify_tiltfile_path', { filePath }),
      openExternal: async (url: string) => { await openUrl(url); },
      ...
      setLoginItemSettings: (openAtLogin: boolean) =>
        invoke('set_login_item', { openAtLogin }),
      onStatusUpdate: (listener: any) => {
        let unlisten: (() => void) | null = null;
        listen('status-update', (e: any) => listener(e.payload)).then((fn: any) => { unlisten = fn; });
        return () => { unlisten?.(); };
      },
      onLogDelta: (listener: any) => {
        let unlisten: (() => void) | null = null;
        listen('log-delta', (e: any) => listener(e.payload)).then((fn: any) => { unlisten = fn; });
        return () => { unlisten?.(); };
      },
      onConfigUpdated: (listener: any) => {
        let unlisten: (() => void) | null = null;
        listen('config-updated', (e: any) => listener(e.payload)).then((fn: any) => { unlisten = fn; });
        return () => { unlisten?.(); };
      },
